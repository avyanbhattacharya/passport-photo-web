...
const pages = [
  ['/clean-html-printer/', 'clean-html-printer/index.html', 'https://cleanlocaltools.com/clean-html-printer/'],
  ...
  ['/batch-image-converter/', 'batch-image-converter/index.html', 'https://cleanlocaltools.com/batch-image-converter/'],
...
const brandedFilePages = ['batch-image-converter/index.html','batch-image-converter/index.html','compress-pdf/index.html','merge-pdf/index.html','clean-pdf-printer/index.html','document-flattener/index.html','image-to-pdf/index.html','split-pdf/index.html','heic-to-jpg/index.html','remove-photo-metadata/index.html'];
...
test('homepage keeps the compact nontechnical private-local brand story',()=>{const homepage=read('index.html');for(const phrase of ['PRIVATE BY DESIGN','Useful file tools.','Your files stay with you.','Keep your files to yourself.','File is sent away','Work happens here.','Nothing is uploaded for processing.','Popular tools','All tools'])assert.ok(homepage.includes(phrase),`homepage missing brand/layout phrase: ${phrase}`);assert.ok(homepage.includes('aria-label="Typical online file tool flow"'));assert.ok(homepage.includes('aria-label="Clean Local Tools flow"'));assert.ok(homepage.includes('aria-label="Your file stays on this device"'));assert.ok(homepage.includes('id="popular-tools"'));assert.ok(homepage.includes('id="all-tools"'));assert.match(homepage,/@media\(max-width:600px\)[\s\S]*\.popular-grid \.tool-card:nth-child\(n\+5\)\{display:none\}/,'mobile should show four popular tools before the full catalog');const popular=homepage.indexOf('id="popular-tools"'),catalog=homepage.indexOf('id="all-tools"'),why=homepage.indexOf('id="why"'),story=homepage.indexOf('aria-label="Typical online file tool flow"');assert.ok(popular>=0&&catalog>popular&&why>catalog&&story>why,'homepage should put the complete tool catalog before the deeper trust story');assert.doesNotMatch(homepage,/WebAssembly|PDF\.js|browser memory|server-side document processing/i,'homepage brand story should stay nontechnical');});
