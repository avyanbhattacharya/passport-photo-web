...
test('print settings reach the isolated document and selection never prints', async ({ page }) => {
  ...
  await expect.poll(() => doc.locator('#printStyles').evaluate(el => el.textContent)).toContain('size:Letter landscape');
  await expect.poll(() => doc.locator('#printStyles').evaluate(el => el.textContent)).toContain('font-size:14pt');
