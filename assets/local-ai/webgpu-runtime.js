(function (root, factory) {
  ...
  // Tiny deterministic neural-network weights used only to validate the runtime.
  // This is a foundation probe, not a user-facing intelligence model.
  ...
  function validateInput(input) {
    if (!Array.isArray(input) && !(input instanceof Float32Array)) {
      throw new TypeError('Model input must be an array of four finite numbers.');
    }
    if (input.length !== INPUT_SIZE) {
      throw new RangeError(`Model input must contain exactly ${INPUT_SIZE} values.`);
    }
    const values = Array.from(input, Number);
    if (!values.every(Number.isFinite)) {
      throw new TypeError('Model input values must be finite numbers.');
    }
    return values;
  }
  ...
  function inferCPU(input) {
    ...
    for (let h = 0; h < HIDDEN_SIZE; h += 1) {
      ...
      for (let i = 0; i < INPUT_SIZE; i += 1) {
        value += x[i] * MODEL.hiddenWeights[h * INPUT_SIZE + i];
      }
    ...
    for (let o = 0; o < OUTPUT_SIZE; o += 1) {
      ...
      for (let h = 0; h < HIDDEN_SIZE; h += 1) {
        value += hidden[h] * MODEL.outputWeights[o * HIDDEN_SIZE + h];
      }
      logits[o] = value;
    }
  ...
  async function createRuntime(options) {
    ...
    const secureContext = Object.prototype.hasOwnProperty.call(opts, 'secureContext')
      ? opts.secureContext
      : (typeof isSecureContext === 'boolean' ? isSecureContext : true);
    ...
    const adapter = await navigatorLike.gpu.requestAdapter({ powerPreference: opts.powerPreference || 'high-performance' });
    if (!adapter) {
    ...
    if (opts.preferFloat16 && adapter.features && adapter.features.has('shader-f16')) {
    ...
    return {
      ...
      device,
      requiredFeatures,
      capabilities: summarizeAdapter(adapter, device),
      lost
    };
  }
    ...
    const shader = device.createShaderModule({ code: `
      ...
      fn main(@builtin(global_invocation_id) id: vec3<u32>) {
        ...
        if (i < arrayLength(&inputData)) {
          outputData[i] = inputData[i] * 2.0 + 1.0;
        }
      }
    ` });
  ...
  async function inferWebGPU(device, input) {
      ...
      fn main() {
        ...
        for (var h: u32 = 0u; h < 4u; h = h + 1u) {
          ...
          for (var i: u32 = 0u; i < 4u; i = i + 1u) {
            value = value + inputData[i] * model[h * 4u + i];
          }
        ...
        for (var o: u32 = 0u; o < 3u; o = o + 1u) {
          ...
          for (var h: u32 = 0u; h < 4u; h = h + 1u) {
            value = value + hidden[h] * model[20u + o * 4u + h];
          }
          outputData[o] = value;
        }
      }
    ...
    pass.end();
    encoder.copyBufferToBuffer(outputBuffer, 0, readBuffer, 0, OUTPUT_SIZE * 4);
    device.queue.submit([encoder.finish()]);
    await readBuffer.mapAsync(mapMode.READ);
