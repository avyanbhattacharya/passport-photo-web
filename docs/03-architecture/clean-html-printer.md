...
The output lives in an iframe sandboxed with `allow-same-origin allow-modals`, without `allow-scripts`. Same-origin permits the parent editor to operate on the reconstructed DOM; modals permit user-requested printing. A document CSP blocks all resources except data images and application-owned inline CSS, and blocks forms and base URLs. Neither sandbox nor CSP substitutes for sanitization. No user HTML is injected into the host application DOM. No persistence, analytics, upload or external dependency is used by this tool.
